...
with st.sidebar:
    ...
    lookback_days = st.slider("Lookback window (days)", 3, 30, 5, key="eqco_lookback")

tab_eq, tab_co = st.tabs(["Equities", "Commodities"])

for tab, asset_code, color in ((tab_eq, "EQUITIES", CATEGORICAL[3]), (tab_co, "COMMODITIES", CATEGORICAL[4])):
    with tab:
